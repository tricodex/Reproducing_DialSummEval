...
factcc_data_path = './reproduce/metrics/factCC/data_to_eval' #'/home/gaomq/factCC/data_to_eval'

def create_eval_jsonl(summary_path, out_jsonl_path):
    documents = list(map(lambda x:x.replace(' | ',' ').strip('|').strip(),open(source_path, encoding='utf8').readlines()))
    summaries = list(map(lambda x:x.strip(),open(summary_path, encoding='utf8').readlines()))
...
def run_eval(DATA_PATH,RESULT_PATH,CKPT_PATH = 'C:\\Users\\patri\\Desktop\\University\\AI\\combots\\Reproducing_DialSummEval\\reproduce\\metrics\\factCC\\factcc-checkpoint'): # change to your path
    #new
    cmd = '''
    python C:\\Users\\patri\\Desktop\\University\\AI\\combots\\Reproducing_DialSummEval\\reproduce\\metrics\\factCC\\my_modeling\\my_eval.py \  # change to your path
