...
def create_tsv(refs_path, hypos_path, out_tsv_path):
    refs = list(map(lambda x:x.strip(),open(refs_path, encoding='utf8').readlines()))
    hypos = list(map(lambda x:x.strip(),open(hypos_path, encoding='utf8').readlines()))
    with open(out_tsv_path,'w', encoding='utf8') as f:
