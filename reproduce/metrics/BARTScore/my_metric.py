...
def print_bartscore(output_fname='bartscore.csv'):
    ...
    bart_scorer.load(path='C:\\Users\\patri\\Desktop\\University\\AI\\combots\\models\\bart_score.pth') # change to your model path
    # out_f = open(output_fname,'w')
    for k,v in tqdm(model_type_dict.items()):
        
        fpath = os.path.join(summ_home_path, v, 'summs.txt')
        summaries = list(map(lambda x:x.strip(),open(fpath, encoding='utf8').readlines()))
